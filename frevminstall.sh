#!/bin/bash
# CtrlPanel Automated Installer (No systemctl Version)
# Version: 1.1.1
...
git clone https://github.com/Ctrlpanel-gg/panel.git ./ || (echo -e "${RED}Failed to clone repository.${RESET}" && exit 1)
...
# Setup queue worker without systemctl
echo -e "${YELLOW}Starting CtrlPanel queue worker...${RESET}"
nohup php /var/www/ctrlpanel/artisan queue:work --sleep=3 --tries=3 > /var/log/ctrlpanel-worker.log 2>&1 &
echo -e "${GREEN}Queue worker started in background.${RESET}"
echo -e "Log file: /var/log/ctrlpanel-worker.log"

# Auto-restart on reboot using cron @reboot
(crontab -l 2>/dev/null; echo "@reboot service redis-server start && nohup php /var/www/ctrlpanel/artisan queue:work --sleep=3 --tries=3 > /var/log/ctrlpanel-worker.log 2>&1 &") | crontab -
